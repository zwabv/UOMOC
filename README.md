...
The core algorithm centers on “semantic refinement-cross-modal fusion-scale adaptation,” employing a category-related weight modulation algorithm to adaptively adjust textual features; An association model linking image regions to category semantics is constructed using cross-attention, capturing global semantics and long-range dependencies. The FFN structure of the Transformer decoder is enhanced through a gating mechanism and receptive field expansion algorithm to strengthen multi-scale feature representation.
During implementation, the CLIP image encoder and text encoder serve as the foundation. First, the text input features are optimized. Next, the optimized text features undergo cross-modal fusion with the visual features output by the image encoder. Subsequently, the fused features are fed into the modified Transformer decoder for feature refinement. Finally, features are aggregated via a global pooling algorithm and processed through an activation function to produce multi-label classification results. This streamlined and efficient workflow effectively addresses the classification demands of complex underwater scenarios.
Training Commands
